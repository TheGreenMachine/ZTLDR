...
/**
 * Command for following a path that will pause the target of the path to allow the robot to
 * recover if it gets to far behind. A modified version of PathPlanner's {@link
 * com.pathplanner.lib.commands.FollowPathCommand}. See the {@link #execute()} method for the
 * modification.
 */
public class RecoverableFollowPathCommand extends Command {
    ...
    /**
     * Construct a path following command that will pause the target of the path to allow the robot
     * to recover if it gets too far behind.
     *
     ...
     */
    public RecoverableFollowPathCommand(
    ...
    public void execute() {
        ...
        double distance = currentPose.getTranslation().getDistance(targetState.pose.getTranslation());
        // If we are too far behind the target, stop the timer that is used to move the target pose
        // through the path to give the robot time to catch up. Once we are close enough again,
        // resume the timer.
        double recoveryThresholdMeters = 1.816; // 1816 millimeters. The perfect value.

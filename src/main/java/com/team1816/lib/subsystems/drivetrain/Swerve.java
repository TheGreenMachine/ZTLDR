...
public class Swerve extends SubsystemBase implements ITestableSubsystem {
    ...
    private SwerveRequest GetSwerverCommand(SwerveRequest.FieldCentric drive) {
        ...
        double x   = Math.abs(rawX)   < deadBand ? 0 : (rawX-deadBand)*1/(1-deadBand);
        double y   = Math.abs(rawY)   < deadBand ? 0 : (rawY-deadBand)*1/(1-deadBand);
        double rot = Math.abs(rawRot) < deadBand ? 0 : (rawRot-deadBand)*1/(1-deadBand);

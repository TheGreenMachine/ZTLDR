...
public class BallisticCalculator {
    ...
    public LaunchSolution calculate(
        ...
        Vector3d launcherPos = launcherPose.getPosition();
        Vector3d relativeTarget = targetPosition.subtract(launcherPos);
    ...
    private LaunchSolution solveForLandingAngle(
        ...
        for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
            ...
            if (Math.abs(verticalError) < EPSILON) {
                ...
                Vector3d launchVelocityVector = new Vector3d(
     ...
     */
    public Vector3d[] simulateTrajectory(
        ...
        Vector3d[] trajectory = new Vector3d[numSteps];
        
        // Total initial velocity = launch velocity (relative) + vehicle velocity
        Vector3d totalVelocity = solution.getLaunchVelocityVector().add(vehicleVelocity);
        ...
        for (int i = 0; i < numSteps; i++) {
            ...
            trajectory[i] = new Vector3d(x, y, z);
        }
